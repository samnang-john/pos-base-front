<template>
  <div class="bg-white p-6 rounded-xl shadow-lg animate-fade-in-down">
    ...
    <div
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <span class="text-sm text-gray-500 bg-gray-50 px-3 py-1.5 rounded-lg">
          Found <span class="font-semibold text-indigo-600">{{ products.total }}</span> products
        </span>
      </div>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-100">
      <table class="w-full">
        <thead class="bg-gray-50">
          <tr>
            ...
            <TableHeaderCell field="title" :sort-field="sortField" :sort-direction="sortDirection"
              @click="sortProducts('title')" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900">
              Title
            </TableHeaderCell>
            <TableHeaderCell field="category" :sort-field="sortField" :sort-direction="sortDirection"
              @click="sortProducts('category')" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900">
              Category
            </TableHeaderCell>
            <TableHeaderCell field="price" :sort-field="sortField" :sort-direction="sortDirection"
              @click="sortProducts('price')" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900">
              Price
            </TableHeaderCell>
            <TableHeaderCell field="cost" :sort-field="sortField" :sort-direction="sortDirection"
              @click="sortProducts('cost')" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900">
              Cost
            </TableHeaderCell>
            <TableHeaderCell field="updated_date" :sort-field="sortField" :sort-direction="sortDirection"
              @click="sortProducts('updated_date')" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900">
              Updated Date
            </TableHeaderCell>
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          <tr v-if="products.loading || !products.data.length">
            <td colspan="8" class="px-3 py-6 text-sm text-gray-500 text-center">
              <Spinner v-if="products.loading" class="mx-auto" />
              <p v-else class="text-center py-8 text-gray-500">
                No products found
              </p>
            </td>
          </tr>
          <tr v-else v-for="(product, index) of products.data" :key="product.id"
            ...
            <td class="whitespace-nowrap px-2 py-2 text-sm text-gray-500">
              {{ new Date(product.updated_date).toLocaleDateString('en-GB', {
                weekday: 'short',
                day: '2-digit',
                month: 'short',
              year: 'numeric'
    ...
    <!-- Pagination -->
    <div v-if="!products.loading" class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
      <div v-if="products.data.length" class="text-sm text-gray-700">
      ...
      <nav v-if="products.total > products.limit" class="flex items-center justify-between px-4 sm:px-0 mt-6">
        <div class="flex flex-1 justify-between sm:justify-end items-center space-x-2">
          <a v-for="(link, i) of products.links" :key="i" :disabled="!link.url" href="#"
            ...
            <span v-if="i === products.links.length - 1" class="sr-only">Next</span>
            <span v-if="i === products.links.length - 1" aria-hidden="true">&raquo;</span>

            <span v-if="i !== 0 && i !== products.links.length - 1" v-html="link.label"></span>
          </a>
        </div>
      </nav>
    </div>
...
function getForPage(ev, link) {
  ...
  const baseUrl = 'http://localhost:8000/api/product/getAllProducts';
...
function deleteProduct(product) {
  ...
  store.dispatch('deleteProduct', product._id)
    .then(res => {
      store.dispatch('getProducts')
    })
}
