...
<template>
  <div class="p-6 rounded-xl min-h-screen text-gray-100">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <!-- Total Products -->
      <div class="bg-white rounded-lg p-6 border border-[#9A6A3A] hover:border-yellow-500/50 transition">
      ...
      <div class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="w-full text-sm">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-4 py-4 text-left font-semibold text-gray-900 sm:px-6">
                {{ $t('TABLE.no') }}
              </th>
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            ...
            <!-- Order Rows -->
            <tr v-else v-for="(order, index) in listOrder" :key="order._id"
              class="hover:bg-gray-50 transition-colors duration-150 cursor-pointer" @click="openModal(order)">
      ...
      <div class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            ...
            <!-- Order Rows -->
            <tr v-else v-for="(history, index) in listHistory" :key="history._id"
              class="hover:bg-gray-50 transition-colors duration-150 cursor-pointer" @click="openModal(history)">
              <td class="px-4 py-4 font-medium text-gray-700 sm:px-6">
                {{ String(index + 1).padStart(2, "0") }}
              </td>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center"
      ...
      <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md" @click.stop>
        <h2 class="text-lg font-bold text-[#9A6A3A] mb-4">{{ $t('MODAL.order_detail') }}</h2>
