<template>
    <div class="bg-white p-6 rounded-xl shadow-lg animate-fade-in-down">
        <div
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                ...
                <span class="text-sm text-gray-500 bg-gray-50 px-3 py-1.5 rounded-lg">
                    Found <span class="font-semibold text-indigo-600">{{ categories.total }}</span> categories
                </span>
            </div>
        ...
        <div class="overflow-x-auto rounded-lg border border-gray-100">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    <tr v-if="categories.loading || !categories.data.length">
                        <td colspan="8" class="px-3 py-6 text-sm text-gray-500 text-center">
                            <Spinner v-if="categories.loading" class="mx-auto" />
                            <p v-else class="text-center py-8 text-gray-500">
                                No categories found
                            </p>
                        </td>
                    </tr>
                    <tr v-else v-for="(category, index) of categories.data" :key="category.id"
                                    ...
                                    <MenuItems
                                        ...
                                        <div class="py-1">
                                            ...
                                            <button :class="[
                                                ...
                                                <TrashIcon class="mr-3 h-4 w-4 text-red-500 group-hover:text-red-700"
                                                    aria-hidden="true" />
                                                Delete
                                            </button>
                                            </MenuItem>
        ...
        <div v-if="!categories.loading" class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
            <div v-if="categories.data.length" class="text-sm text-gray-700">
            ...
            <nav v-if="categories.total > categories.limit" class="flex items-center justify-between px-4 sm:px-0 mt-6">
                <div class="flex flex-1 justify-between sm:justify-end items-center space-x-2">
                    <a v-for="(link, i) of categories.links" :key="i" :disabled="!link.url" href="#"
                        ...
                        <span v-if="i === categories.links.length - 1" class="sr-only">Next</span>
                        <span v-if="i === categories.links.length - 1" aria-hidden="true">&raquo;</span>

                        <span v-if="i !== 0 && i !== categories.links.length - 1" v-html="link.label"></span>
                    </a>
                </div>
            </nav>
        </div>
...
function getForPage(ev, link) {
  ...
  const baseUrl = 'http://localhost:8000/api/category/getAllCategories';
