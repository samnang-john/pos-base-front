...
const isFormValid = computed(() => {
  return woodLengthNameForm.value.name.trim() !== "";
});

onMounted(async () => {
  try {
    await Promise.all([getWoodLengths()]);
  } catch (error) {
    console.error("Error during initialization:", error);
  }
});

const getWoodLengths = async (page = 1) => {
...
const nextPage = () => {
  if (currentPage.value < totalPages.value) {
    getWoodLengths(currentPage.value + 1);
  }
};
...
const submitWoodLength = async () => {
  if (woodLengthNameForm.value.name !== "") {
    try {
      await store.dispatch("createWoodLength", woodLengthNameForm.value);
      getWoodLengths();
      closeModal();
      toast.success(this.$t('TOAST.wood_length_created'));
    } catch (error) {
      closeModal();
      console.log("Error=>", error);
      toast.error(this.$t('TOAST.wood_length_unsuccessful'));
    }
  } else {
...
const onUpdate = async () => {
  try {
    const obj = {
      ...
      name: woodLengthNameForm.value?.name,
      description: woodLengthNameForm?.value?.description,
    };
    await store.dispatch("updateWoodLength", obj);
    getWoodLengths();
    closeModal();
    toast.success(this.$t('TOAST.wood_length_updated'));
  } catch (error) {
    console.log("Error=>", error);
    toast.error(this.$t('TOAST.wood_length_unsuccessful'));
  }
...
const onDeleteWoodLength = async (woodLengthId) => {
  try {
    await store.dispatch("deleteWoodLength", woodLengthId);
    getWoodLengths();
    toast.info(this.$t('TOAST.wood_length_deleted'));
  } catch (error) {
    toast.error(this.$t('TOAST.wood_length_unsuccessful'));
  }
};
...
<template>
  ...
  <div class="flex items-center justify-between mb-3">
    <h1 class="text-3xl font-semibold">{{ $t('MENU.wood_length') }}</h1>
    ...
    <div
      ...
      <div class="relative w-full md:w-auto">
        ...
        <input v-model="search" @change="getProducts(null)"
          ...
          :placeholder="$t('TABLE.search_wood_length')" />
      </div>
    </div>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-100">
      <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          <tr v-if="isLoadingWoodLength || !listWoodLengths.length">
            <td colspan="8" class="px-3 py-6 text-sm text-gray-500 text-center">
              <Spinner v-if="isLoadingWoodLength" class="mx-auto" />
              <p v-else class="text-center py-8 text-gray-500">
                {{ $t('TABLE.no_wood_length_found') }}
              </p>
            </td>
          </tr>
          <tr v-else v-for="(wood, index) of listWoodLengths" :key="wood._id"
                  ...
                  <MenuItems
                    ...
                    <div class="py-1">
                      ...
                      <button :class="[
                        ...
                        'group flex w-full items-center px-4 py-2 text-sm',
                      ]" @click="onDeleteWoodLength(wood._id)">
  ...
  <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center"
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md" @click.stop>
      <h2 v-if="isUpdate" class="text-lg font-bold text-gray-800 mb-4">
        {{ $t('update_wood_length') }}
      </h2>
      <h2 v-else class="text-lg font-bold text-gray-800 mb-4">
        {{ $t('create_wood_length') }}
      </h2>
      <div class="mb-4">
        <label for="woodLengthName" class="block text-sm font-medium text-gray-700">{{ $t('TABLE.wood_length') }}
          <span class="text-red-500">*</span>
        </label>
        <input v-model="woodLengthNameForm.name" id="woodLengthName" type="text" :class="[
          'mt-1 w-full p-2 border rounded-lg focus:ring-[#986b41] focus:border-[#986b41]',
          isErrorValue ? 'border-red-500' : 'border-gray-300',
        ]" :placeholder="$t('TABLE.enter_wood_length')" />
      </div>
      <div class="mb-4">
        <label for="note" class="block text-sm font-medium text-gray-700">{{ $t('TABLE.note') }}</label>
        <input v-model="woodLengthNameForm.description" id="note" type="text"
      ...
      <div class="flex justify-end space-x-2">
        ...
        <button v-else @click="submitWoodLength"
          class="px-4 py-2 text-white bg-[#986b41] rounded-lg hover:bg-[#B68E65] focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
          :disabled="!isFormValid">
          {{ $t('BUTTON.create') }}
        </button>
